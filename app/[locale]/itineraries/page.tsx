...
import type { TravelerType, TravelStyle, PlannerInput, Locale } from "@/types";
...
function ItinerariesContent() {
  ...
  const [filters, setFilters] = useState<PlannerInput & { country?: string }>({
    destination: "",
    duration: "",
    travelerType: "",
    style: "",
  ...
  useEffect(() => {
    ...
    setFilters({
      ...
      travelerType: (searchParams.get("travelerType") as TravelerType) ?? "",
      style: (searchParams.get("style") as TravelStyle) ?? "",
      country,
    });
  }, [searchParams]);
  ...
  const filtered = useMemo(() => {
    return itineraries.filter((itin) => {
      if (filters.destination && itin.destination !== filters.destination) return false;
      if (filters.country) {
      ...
      if (filters.style && itin.style !== filters.style) return false;
      return true;
    });
  }, [filters]);
  ...
  function updateFilter(key: string, value: string) {
    ...
    if (key === "country") next.destination = "";
    setFilters(next);
  ...
  function clearFilters() {
    setFilters({ destination: "", duration: "", travelerType: "", style: "" });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 mb-8 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <select className={selectBase} value={filters.destination || filters.country || ""} onChange={(e) => {
            const val = e.target.value;
            const isCountry = countries.some((c) => c.id === val);
            if (isCountry) {
              updateFilter("country", val);
            } else {
              const next = { ...filters, country: undefined };
              setFilters(next);
              updateFilter("destination", val);
            }
          }}>
          ...
          <select className={selectBase} value={filters.duration} onChange={(e) => updateFilter("duration", e.target.value)}>
            ...
            {durationOptions.map((d) => (
              <option key={d.value} value={d.value}>{d.label[locale]}</option>
            ))}
          </select>
          ...
          <select className={selectBase} value={filters.travelerType} onChange={(e) => updateFilter("travelerType", e.target.value)}>
            ...
            {travelerTypeOptions.map((tt) => (
              <option key={tt.value} value={tt.value}>{tt.emoji} {tt.label[locale]}</option>
            ))}
          </select>
          ...
          <select className={selectBase} value={filters.style} onChange={(e) => updateFilter("style", e.target.value)}>
            ...
            {styleOptions.map((s) => (
              <option key={s.value} value={s.value}>{s.label[locale]}</option>
            ))}
          </select>
        </div>
      </div>
